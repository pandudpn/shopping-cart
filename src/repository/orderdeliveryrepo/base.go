...
const (
	...
	QUERY_SELECT = "select od.id, od.delivery_cost, od.delivery_cost_discount, od.total_delivery_cost, od.tracking_number, od.address, " +
		...
		"ua.post_code, pr.id, pr.name, ct.id, ct.name, dt.id, dt.name, a.id, a.name, od.rate " +
...
func rowToOrderDelivery(row *sql.Row) (*model.OrderDelivery, error) {
	...
	err := row.Scan(
		...
		&province.Id, &province.Name, &city.Id, &city.Name, &district.Id, &district.Name, &area.Id, &area.Name, &orderDelivery.Rate,
	)
	if err != nil {
		return nil, err
	}
