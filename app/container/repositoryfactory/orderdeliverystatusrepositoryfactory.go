...
import (
	...
	"github.com/pandudpn/shopping-cart/src/repository/orderdeliverystatusrepo"
)

type orderDeliveryStatusRepositoryFactory struct{}

func (odsrf *orderDeliveryStatusRepositoryFactory) Build(c container.Container) (RepositoryFactoryInterface, error) {
	code := constant.PSQL
	...
	db := dsfi.(dbc.SqlDbc)
	odsri := orderdeliverystatusrepo.OrderDeliveryStatusRepository{DB: db}
